...
public class Aerolinea
{
    ...
    public void programarVuelo(String fecha, String codigoRuta, String nombreAvion) throws Exception {
        ...
        if (avionSeleccionado == null) {
            throw new Exception("El avión con el nombre " + nombreAvion + " no existe.");
        }

        for (Vuelo vuelo : vuelos) {
            if (vuelo.getFecha().equals(fecha) && vuelo.getAvion().getNombre().equals(nombreAvion)) {
                throw new Exception("El avión " + nombreAvion + " ya está programado para otro vuelo en la fecha " + fecha + ".");
            }
        }
    ...
    public int venderTiquetes(String identificadorCliente, String fecha, String codigoRuta, int cantidad) throws VueloSobrevendidoException, Exception {
        ...
        if (cliente == null) {
            throw new Exception("El cliente con el identificador " + identificadorCliente + " no existe.");
        }
        ...
        // Verifica si hay suficiente espacio en el vuelo para los pasajeros
    	if (cantidad > vuelo.getAvion().getCapacidad()) {
    		throw new VueloSobrevendidoException(vuelo);
    	}

        int mesVuelo = Integer.parseInt(fecha.split("-")[1]); // Suponiendo que el formato de la fecha es "YYYY-MM-DD"
        boolean esTemporadaAlta = (mesVuelo >= 6 && mesVuelo <= 8) || mesVuelo == 12;
        ...
        int costoTotal = tarifa * cantidad;
    ...
    public void registrarVueloRealizado(String fecha, String codigoRuta) {
    	List<Vuelo> vuelosRealizados = new ArrayList <Vuelo>();
        Vuelo vuelo = getVuelo(codigoRuta, fecha);
        if (vuelo != null) {
    ...
    public String consultarSaldoPendienteCliente(String identificadorCliente) {
        Cliente cliente = getCliente(identificadorCliente);
        if (cliente == null) {
            return "El cliente con el identificador " + identificadorCliente + " no existe.";
        }
        
        int valor = cliente.calcularValorTotalTiquetes();

        return "El saldo pendiente del cliente " + identificadorCliente + " es: " + valor;
    }
